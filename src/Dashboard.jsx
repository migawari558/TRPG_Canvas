...
import { Search, Plus, Upload, FileText, Network, ArrowUpRight, RefreshCw, Feather, FolderOpen } from 'lucide-react';

export default function Dashboard({ documents, workspace, busy, onOpen, onCreate, onImport, onRefresh }) {
  ...
  return <section className="dashboard" aria-label="シナリオダッシュボード">
    ...
    <div className="scenario-grid">{filtered.map(doc => <button className="scenario-card" key={doc.id} disabled={busy} onClick={() => onOpen(doc.id)}>
