...
export default function ScenarioEditor({ doc, onChange, onReady }) {
  ...
  const syntax = ['**文字**', '*文字*', '<u>文字</u>', 'Ctrl+F / Ctrl+H', '#! ＋ Space', '# ＋ Space', '## ＋ Space', '### ＋ Space', '- ＋ Space', '1. ＋ Space', '- [ ] ＋ Space', '> ＋ Space', '!!! ＋ Spaceで開始 / Ctrl+Enterで終了（Markdown: :::gm ～ :::）', '画像ファイルを選択・貼り付け', '\`\`\` ＋ Space', '---', 'Ctrl+Z', 'Ctrl+Shift+Z'];
  const controls = [
  ...
  return <><div className="editor-toolbar"><span className="toolbar-label">本文</span><span className="toolbar-divider"/>{controls.map(([Icon, label, action, active], index) => <button key={label} disabled={imageBusy} title={`${label}\n記法: ${syntax[index]}`} aria-label={label} aria-pressed={!!active} className={active ? 'active' : ''} onClick={action}><Icon size={16}/></button>)}<span className="markdown-badge">Markdown</span></div>{findOpen && <FindBar editor={editor} request={findRequest} onClose={() => setFindOpen(false)}/>}<input className="hidden" ref={imageInput} type="file" accept="image/png,image/jpeg,image/gif,image/webp" multiple aria-label="挿入する画像" onChange={event => { const files = [...event.target.files]; event.target.value = ''; insertImages(files); }}/>{imageBusy && <div className="image-status" role="status">画像を読み込み中…</div>}{imageError && <div className="image-error" role="alert">{imageError}<button aria-label="画像エラーを閉じる" onClick={() => setImageError('')}><X size={16}/></button></div>}<div className="editor-scroll" onPaste={e => { if (e.defaultPrevented || e.clipboardData.files.length) return; const text = e.clipboardData.getData('text/plain'); if (!e.clipboardData.getData('text/html') && /^(:::gm|#! |#{1,6} |```|> |\- )/m.test(text)) { e.preventDefault(); editor.commands.insertContent(markdown.render(text)); } }}><article className="paper"><div className="paper-eyebrow"><span/> SCENARIO MANUSCRIPT</div><EditorContent editor={editor}/><div className="paper-end">◆</div></article></div></>;
}
