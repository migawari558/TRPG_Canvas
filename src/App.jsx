...
export default function App() {
  ...
  return <div className={`app ${sidebar ? '' : 'sidebar-hidden'}`}>
    ...
    {modal === 'help' && <Modal title="書くことに、集中しよう。" eyebrow="QUICK GUIDE" onClose={() => setModal(null)}><p className="modal-description">Markdown記法が、入力したその場で整います。見出し・引用・リストは記号のあとにスペースを入力してください。</p><div className="shortcut-list">{[['## + Space', '章見出し'], ['### + Space', '小見出し'], ['> + Space', '共有情報・読み上げ文'], ['- + Space', '箇条書き'], ['**テキスト**', '太字'], ['Ctrl + Z', '元に戻す'], ['Ctrl + S', 'すぐに保存']].map(([key, value]) => <div key={key}><code>{key}</code><span>{value}</span></div>)}</div><p className="hint-box">目次の矢印は、小見出しと本文もまとめて移動します。フローの鉛筆ボタンで「対応する目次」を選ぶと、シーンのクリックで本文へ移動できます。「目次でグループ化」で、章と小見出しを入れ子に配置します。シーン下側の点から、次のシーン上側の点へドラッグして接続できます。</p></Modal>}
